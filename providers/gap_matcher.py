...
def _build_prompt(requirements: list[str], cv_text: str, candidate_notes: str) -> str:
    ...
    return (
        "You are an honest career advisor assessing whether a candidate is truly qualified "
        "for a specific role. Your job is to PROTECT the candidate from wasting time on "
        "poor-fit applications.\n\n"
        "Match each job requirement against the candidate's background. "
        "Do not rely on exact keyword matching, but DO consider the DEPTH and CONTEXT "
        "of experience — surface familiarity is not the same as professional expertise.\n\n"
        "Strength definitions (apply these strictly):\n"
        "- strong: The candidate has DIRECT, professional-level experience at the depth "
        "this role demands. They could perform this on day one. "
        "Example: A data scientist with 3 years of ML model building is 'strong' for "
        "a machine learning requirement.\n"
        "- partial: The candidate has TRANSFERABLE or ADJACENT experience, but not at the "
        "depth or specificity the role expects. They would need meaningful ramp-up. "
        "Example: A product manager who uses SQL for dashboards is 'partial' for a "
        "data science SQL requirement that involves complex statistical queries.\n"
        "- gap: The candidate has NO meaningful experience in this area, or only trivial "
        "exposure. Do not stretch unrelated experience to fill gaps.\n\n"
        "CRITICAL: If the candidate's core role/discipline is fundamentally different "
        "from the target role (e.g. product manager applying to data scientist, "
        "marketer applying to software engineer), most technical requirements should be "
        "'partial' or 'gap' — not 'strong' — unless the candidate has genuinely deep "
        "experience in that specific skill.\n\n"
        "Rules:\n"
        ...
        "- rationale must be specific, 1-2 sentences, and must explain WHY the strength "
        "level was chosen (especially for 'partial' — what's missing?).\n"
        "- match_strength must be one of: strong, partial, gap.\n"
        "- Be honest and calibrated. A score that's too generous wastes the candidate's time.\n\n"
